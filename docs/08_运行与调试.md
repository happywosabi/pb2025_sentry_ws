...
> 向后兼容：不传 `use_sp_vision:=True` 时将走旧 `pb2025_rm_vision` 链路。
...
graph TB
    ...
    B --> C[启动串口通信]
    C --> D{use_sp_vision?}
    D -->|True| E[启动 sp_vision_25（原生相机）]
    D -->|False| E2[启动 hik_camera + pb2025_rm_vision]
    E --> F[启动LiDAR驱动]
    E2 --> F
    F --> G[启动导航模块]
    G --> H[启动行为树]
    ...
    class C hardwareClass
    class E,E2 perceptionClass
    class F,G navClass
    class H decisionClass
    class A,B,D,I,J processClass
```
...
> sp_vision_25 主模式下**不要**启动 hik_camera_ros2_driver（sp_vision 独占 USB）。以下仅适用于旧视觉模式或独立验证相机。
...
**主方案：sp_vision_25**（原生相机 + YOLO + EKF + 弹道 + 自主射击）：

```bash
# 独立启动 sp_vision_25（会独占相机）
...
ros2 topic hz /cmd_gimbal      # 期望 ~165 Hz

# 启用调试可视化（在 sentry.yaml 中 debug.enable_visualization: true）
ros2 run rqt_image_view rqt_image_view /sentry_debug/image
```

调优点：`enemy_color`、`exposure_ms`、`gain`、`min_confidence`、`yaw_offset`、`pitch_offset`。完整参数见 [docs/07_参数配置.md](07_参数配置.md#3-感知层参数sp_vision_25-主)。

---

**旧方案：pb2025_rm_vision**（已废弃，仅 `use_sp_vision:=False` 时使用）：
...
**问题**：sp_vision_25 报 *Camera open failed* / hik_camera 启动失败

- sp_vision_25 与 `hik_camera_ros2_driver` **不能同时运行**（独占 USB）
- 主模式下不要启动 hik_camera_ros2_driver，启动参数 `use_sp_vision:=True` 会自动跳过
...
调试步骤：
```bash
# 1. 启用可视化（src/sp_vision_25/configs/sentry.yaml）
# debug.enable_visualization: true
ros2 run rqt_image_view rqt_image_view /sentry_debug/image

# 2. 检查关键参数（sentry.yaml）
# enemy_color: "red" 或 "blue"
# exposure_ms: 8         (偏暗增大，偏亮减小)
# gain: 16.9
# min_confidence: 0.8    (漏检高可调低到 0.7)

# 3. 瘦准偏移 → 修改 yaw_offset / pitch_offset（度）
# 详见 docs/13_四元数转换与参数调优.md
...
**旧模式（pb2025_rm_vision，`use_sp_vision:=False`）**
...
**视觉检测**：
- sp_vision_25：调低 `min_confidence`（0.8→0.7）减少漏检；调整 `exposure_ms` / `gain` 适配光照
- 使用 OpenVINO GPU 加速（在 sentry.yaml 中设 `device: GPU`）
- 旧模式：降低相机帧率（165Hz → 120Hz）可减少CPU占用
...
echo -e "\n=== 话题频率 ==="
ros2 topic hz /cmd_gimbal --once --wait-for-data --timeout 5
ros2 topic hz /Odometry --once --wait-for-data --timeout 5
# 旧视觉模式额外可监控:
