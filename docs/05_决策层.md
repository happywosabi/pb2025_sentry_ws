...
flowchart TB
    ...
    subgraph SPV_L["感知射击层 sp_vision_25 ~165 Hz"]
        CAM["相机 HikRobot SDK 直采"] --> SPV["sp_vision_25<br>YOLO + EKF + 弹道 + 自动开火"]
        SPV --> GIM_CMD["/cmd_gimbal"]
        SPV --> SHOOT_CMD["/cmd_shoot"]
    end

    subgraph NAV_L["导航决策层 20 Hz"]
        NAV_CMD --> NAV2[Nav2 栈]
        NAV2 --> PLAN["路径规划 Theta*"]
        PLAN --> CTRL["运动控制 Omni PID"]
        CTRL --> CHASSIS_CMD["/cmd_vel 底盘"]
    end
